...
class BQ27220{
public:
    // BQ27220() : __addr{BQ27220_I2C_ADDRESS}, __wire(&Wire), __scl(DEFAULT_SCL), __sda(DEFAULT_SDA)
    // {}

    // bool begin()
    // {
    //     return __wire->begin(DEFAULT_SDA, DEFAULT_SCL);
    // }

    bool begin(TwoWire &w = Wire, int sda = -1, int scl = -1, uint8_t addr = BQ27220_I2C_ADDRESS)
    {
        __sda = sda;
        __scl = scl;
        __wire = &w;
        __addr = addr;
        return __wire->begin(__sda, __scl);
    }
    ...
    bool i2cReadBytes(uint8_t reg, uint8_t * dest, uint8_t count) {
        ...
        __wire->write(reg);
        __wire->endTransmission(true);

        __wire->requestFrom(__addr, count);
