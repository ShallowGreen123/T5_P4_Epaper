...
static const char *TAG = "T5-P4-E-Paper";

static bool i2c_initialized = false;
static i2c_master_bus_handle_t i2c_bus_handle;
static i2c_master_dev_handle_t pca9535_handle = NULL;
static bool pca9535_initialized = false;
static uint8_t pca9535_output_state[2] = {0};
static uint8_t pca9535_config_state[2] = {0xFF, 0xFF};
...
static esp_err_t pca9535_write_registers(uint8_t reg, const uint8_t *data, size_t size)
{
    uint8_t payload[3] = {0};

    if (pca9535_handle == NULL || data == NULL || size == 0 || size > 2) {
        return ESP_ERR_INVALID_ARG;
    }

    payload[0] = reg;
    memcpy(&payload[1], data, size);
    return i2c_master_transmit(pca9535_handle, payload, size + 1, 100);
}

static esp_err_t pca9535_init(void)
{
    if (pca9535_initialized) {
        return ESP_OK;
    }
    ...
    i2c_device_config_t dev_cfg = {
        ...
        .device_address = BSP_PCA9535_I2C_ADDR,
        .scl_speed_hz = CONFIG_BSP_I2C_CLK_SPEED_HZ,
    };
    BSP_ERROR_CHECK_RETURN_ERR(i2c_master_bus_add_device(i2c_bus_handle, &dev_cfg, &pca9535_handle));

    esp_err_t ret = i2c_master_probe(i2c_bus_handle, BSP_PCA9535_I2C_ADDR, 100);
    if (ret != ESP_OK) {
        i2c_master_bus_rm_device(pca9535_handle);
        pca9535_handle = NULL;
        return ret;
    }

    pca9535_output_state[0] = 0;
    pca9535_output_state[1] = 0;
    pca9535_config_state[0] = 0xFF;
    pca9535_config_state[1] = 0xFF;

    BSP_ERROR_CHECK_RETURN_ERR(pca9535_write_registers(PCA9535_OUTPUT_PORT_0_REG, pca9535_output_state,
                                                        sizeof(pca9535_output_state)));
    BSP_ERROR_CHECK_RETURN_ERR(pca9535_write_registers(PCA9535_CONFIG_PORT_0_REG, pca9535_config_state,
                                                        sizeof(pca9535_config_state)));

    pca9535_initialized = true;
    return ESP_OK;
}

static esp_err_t pca9535_set_level(uint8_t io_num, bool level)
{
    if (io_num >= 16) {
        return ESP_ERR_INVALID_ARG;
    }

    BSP_ERROR_CHECK_RETURN_ERR(pca9535_init());

    const uint8_t port = io_num / 8;
    const uint8_t bit = 1U << (io_num % 8);

    if (level) {
        pca9535_output_state[port] |= bit;
    } else {
        pca9535_output_state[port] &= (uint8_t)~bit;
    }
    pca9535_config_state[port] &= (uint8_t)~bit;
...
static esp_err_t bsp_audio_poweramp_enable(bool enable)
{
    BSP_ERROR_CHECK_RETURN_ERR(pca9535_set_level(BSP_PCA9535_AUDIO_SHDN, enable));
...
{
    if (pca9535_handle != NULL) {
        BSP_ERROR_CHECK_RETURN_ERR(i2c_master_bus_rm_device(pca9535_handle));
        pca9535_handle = NULL;
        pca9535_initialized = false;
    }
...
esp_codec_dev_handle_t bsp_audio_codec_speaker_init(void)
{
    ...
    assert(i2s_data_if);
    BSP_ERROR_CHECK_RETURN_NULL(bsp_audio_poweramp_enable(true));
