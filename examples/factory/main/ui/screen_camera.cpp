...
constexpr size_t kPhotoPitch = (kPhotoWidth + 7U) / 8U;
constexpr size_t kPhotoBytes = kPhotoPitch * kPhotoHeight;
constexpr size_t kPaletteBytes = sizeof(lv_color32_t) * 2U;
...
static uint8_t *s_photo_map = nullptr;
static uint8_t *s_pending_1bpp = nullptr;
...
static bool ensure_runtime()
{
    ...
    const uint32_t image_size = lv_img_buf_get_img_size(kPhotoWidth, kPhotoHeight, LV_IMG_CF_INDEXED_1BIT);
    if (image_size < kPaletteBytes + kPhotoBytes) {
        ESP_LOGE(TAG, "unexpected LVGL indexed image size: %" PRIu32, image_size);
        return false;
    }

    if (s_photo_map == nullptr) {
        s_photo_map = static_cast<uint8_t *>(alloc_psram(image_size));
    }
    if (s_pending_1bpp == nullptr) {
        s_pending_1bpp = static_cast<uint8_t *>(alloc_psram(kPhotoBytes));
    }
    if (s_photo_map == nullptr || s_pending_1bpp == nullptr) {
        ESP_LOGE(TAG, "failed to allocate camera photo buffers");
        return false;
    }

    memset(s_photo_map, 0xFF, image_size);
    memset(s_pending_1bpp, 0xFF, kPhotoBytes);

    memset(&s_photo_dsc, 0, sizeof(s_photo_dsc));
    s_photo_dsc.header.cf = LV_IMG_CF_INDEXED_1BIT;
    s_photo_dsc.header.always_zero = 0;
    s_photo_dsc.header.reserved = 0;
    s_photo_dsc.header.w = kPhotoWidth;
    s_photo_dsc.header.h = kPhotoHeight;
    s_photo_dsc.data_size = image_size;
    s_photo_dsc.data = s_photo_map;
    lv_img_buf_set_palette(&s_photo_dsc, 0, lv_color_black());
    lv_img_buf_set_palette(&s_photo_dsc, 1, lv_color_white());
...
static void capture_task(void *arg)
{
    ...
    set_camera_statusf(CameraState::Processing,
                       "Dithering to %" PRIu32 "x%" PRIu32 " 1bpp...",
                       kPhotoWidth,
                       kPhotoHeight);
    if (!factory_camera_process_frame(
            raw_frame,
            frame.width,
            frame.height,
            frame.pixel_format,
...
static void save_btn_event_cb(lv_event_t *e)
{
    ...
    uint8_t *copy = static_cast<uint8_t *>(malloc(kPhotoBytes));
    if (copy == nullptr) {
    ...
    if (can_save) {
        memcpy(copy, s_photo_map + kPaletteBytes, kPhotoBytes);
    ...
    if (factory_sd_enter_root()) {
        ...
        if (fp != nullptr) {
            ...
            for (size_t i = 0; i < kPhotoBytes; ++i) {
...
static void refresh_camera_ui()
{
    ...
    if (s_pending_photo) {
        memcpy(s_photo_map + kPaletteBytes, s_pending_1bpp, kPhotoBytes);
    ...
    if (s_photo_img != nullptr && s_placeholder != nullptr) {
        if (has_photo) {
            ...
            if (pending_photo) {
                lv_img_set_src(s_photo_img, &s_photo_dsc);
