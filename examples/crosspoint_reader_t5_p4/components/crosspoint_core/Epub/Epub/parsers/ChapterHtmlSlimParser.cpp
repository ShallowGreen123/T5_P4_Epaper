...
void XMLCALL ChapterHtmlSlimParser::endElement(void* userData, const XML_Char* name) {
  ...
  if (self->insideFootnoteLink && self->depth == self->footnoteLinkDepth) {
    if (self->currentFootnote.number[0] != '\0' && self->currentFootnote.href[0] != '\0') {
      FootnoteEntry entry;
      const size_t numberLen = strnlen(self->currentFootnote.number, sizeof(entry.number) - 1);
      memcpy(entry.number, self->currentFootnote.number, numberLen);
      entry.number[numberLen] = '\0';
      const size_t hrefLen = strnlen(self->currentFootnote.href, sizeof(entry.href) - 1);
      memcpy(entry.href, self->currentFootnote.href, hrefLen);
      entry.href[hrefLen] = '\0';
      int wordIndex =
