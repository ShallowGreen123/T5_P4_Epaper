...
UiWifiState g_wifi_state = UI_WIFI_STATE_ERROR;
int g_wifi_scan_count = 0;
char g_wifi_state_text[64] = "WiFi disabled";
char g_wifi_summary[128] = "Pure ESP-IDF demo: WiFi scan is not enabled.";
char g_wifi_ssid[64] = "Not connected";
char g_wifi_pwd[64] = "-";
char g_wifi_ip[64] = "Not available";
...
void ui_wifi_set_state(UiWifiState state, const char *text) {
    g_wifi_state = state;
    copy_text(g_wifi_state_text, sizeof(g_wifi_state_text), text);
}

void ui_wifi_set_placeholder() {
    ui_wifi_clear_items();
    g_wifi_status = false;
    ui_wifi_set_state(UI_WIFI_STATE_ERROR, "WiFi scan disabled");
    copy_text(g_wifi_summary,
              sizeof(g_wifi_summary),
              "Pure ESP-IDF demo: esp_wifi_remote/esp_hosted scan is not enabled.");
}
}  // namespace
...
const char *ui_wifi_get_ip(void)
{
    return g_wifi_ip;
}
...
{
    // return bq27220.getCharingFinish();
    return 0;
}
