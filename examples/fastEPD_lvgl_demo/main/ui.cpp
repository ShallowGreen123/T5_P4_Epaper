...
static const char *line_full_format(int max_c, const char *str1, const char *str2)
{
    ...
    if(buf_limit <= 0 || buf_limit > GLOBAL_BUF_LEN) {
        buf_limit = GLOBAL_BUF_LEN;
    }
    ...
    for(j = len1; j < buf_limit - 1 - len2; j++){
        global_buf[j] = ' ';
    }
    if(j + len2 > buf_limit - 1) {
        len2 = buf_limit - 1 - j;
    }
...
static void menu_get_gesture_dir(int dir)
{
    ...
    printf("[gesture] curr=%d, sum=%d, dir=%d\n", page_curr, page_num, dir);
...
static void menu_btn_event(lv_event_t *e)
{
    ...
    if(e->code == LV_EVENT_CLICKED) {
        ...
        if(data < ARRAY_LEN(icon_buf))
        {
            ...
            int icon2_index = data - ARRAY_LEN(icon_buf);
            if(icon2_index >= 0 && icon2_index < ARRAY_LEN(icon_buf2)) {
                printf("[%d] %s is clicked.\n", data, icon_buf2[icon2_index].icon_str);
            }
        }
        /************* page1 ************
         * 0 --- SCREEN1_ID  --- clock
...
{
    uint8_t h=0, m=0, s=0;
    uint8_t year=0, mont=0, day=0, week=0;
...
{
    char buf[16] = {0};
    size_t label_len = text_len > 0 ? (size_t)text_len : strlen(text);
    size_t copy_len = label_len < sizeof(buf) ? label_len : sizeof(buf) - 1;
    memcpy(buf, text, copy_len);
    buf[copy_len] = '\0';

    char *suffix = (char *)text + (label_len >= 4 ? label_len - 4 : 0);
    if(label_len >= 4 && label_len - 4 < sizeof(buf)) {
        buf[label_len - 4] = '\0';
    }
...
void ui_auto_timer_cb(lv_timer_t *t)
{
    ...
    if(cursor++ > sizeof(menu_buf)/sizeof(menu_buf[0])) {
        cursor = 0;
    }
}
