...
{
    ESP_RETURN_ON_ERROR(bsp_i2c_init(), TAG, "init shared I2C failed");
    s_i2c_bus = bsp_i2c_get_handle();
    ESP_RETURN_ON_FALSE(s_i2c_bus != NULL, ESP_ERR_INVALID_STATE, TAG, "shared I2C handle unavailable");
...
static void init_touch()
{
    ...
    esp_err_t err = t5_board_touch_new(DISP_WIDTH, DISP_HEIGHT, &s_touch, &s_touch_io, NULL);
    if (err != ESP_OK) {
