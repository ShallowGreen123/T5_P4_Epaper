...
bool Epub::parse_content_opf(ZipFile &zip, std::string &content_opf_file)
{
  ...
  if (!contents)
  {
    ESP_LOGE(TAG, "Could not read content.opf file %s", content_opf_file.c_str());
    return false;
  }
  // parse the contents
  ...
  m_title = title->GetText() ? title->GetText() : m_path;
  ...
  while (item)
  {
    const char *item_id_attr = item->Attribute("id");
    const char *href_attr = item->Attribute("href");
    if (!item_id_attr || !href_attr)
    {
      item = item->NextSiblingElement("item");
      continue;
    }
    std::string item_id = item_id_attr;
...
bool Epub::parse_toc_ncx_file(ZipFile &zip)
{
  ...
  while (navPoint)
  {
    ...
    auto navLabelParent = navPoint->FirstChildElement("navLabel");
    auto navText = navLabelParent ? navLabelParent->FirstChildElement("text") : nullptr;
    auto navLabel = navText ? navText->FirstChild() : nullptr;
...
std::string &Epub::get_spine_item(int spine_index)
{
  ...
  if (spine_index < 0 || spine_index >= (int)m_spine.size())
  {
    ...
    spine_index = std::clamp(spine_index, 0, (int)m_spine.size() - 1);
  }
...
{
  if (toc_index < 0 || toc_index >= (int)m_toc.size())
  {
    return 0;
  }
