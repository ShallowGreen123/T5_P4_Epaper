...
bool EpubList::load(const char *path)
{
  ...
  else
  {
    ...
    renderer->draw_text_box("No readable EPUB directory was found on the SD card.",
                            24, y, renderer->get_page_width() - 48, 100, true, false);
    renderer->draw_text_box(path, 24, y + 80, renderer->get_page_width() - 48, 60, false, false);
...
void EpubList::render()
{
  ...
  {
    renderer->clear_screen();
    renderer->draw_text_box("No EPUB files found.\nCopy books to the SD card and retry.",
                            24, renderer->get_page_height() / 2 - 60,
                            renderer->get_page_width() - 48, 120, true, false);
    state.previous_selected_item = -1;
    state.previous_rendered_page = 0;
    return;
  }
  // what page are we on?
