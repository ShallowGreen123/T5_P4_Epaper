...
void EpubToc::next()
{
  ...
  if (!epub || epub->get_toc_items_count() <= 0)
  {
    return;
  }
...
void EpubToc::render()
{
  ...
  if (!epub || epub->get_toc_items_count() <= 0)
  {
    ...
    renderer->draw_text_box("This book does not expose a table of contents.\nTap Select to open it from the beginning.",
                            24, renderer->get_page_height() / 2 - 60,
                            renderer->get_page_width() - 48, 140, true, false);
