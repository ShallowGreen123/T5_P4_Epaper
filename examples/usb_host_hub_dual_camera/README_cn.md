...
## 故障排除
- 如果检测到摄像头但其被拒绝使用，请确认其确为支持 MJPEG 格式的真 UVC 设备，因为此演示并不处理所有 USB 视频格式。
- 如果设备枚举不稳定，请使用带电源的集线器，并检查 OTG 连接线和电源预算。
- 如果网页加载成功但没有图像显示，请检查摄像头模式选择日志，并确保所选的流索引与有效的 MJPEG 模式相匹配。
